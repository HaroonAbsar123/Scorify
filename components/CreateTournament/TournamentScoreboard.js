...
export default function TournamentScoreboard({navigation, route}) {
  ...
  function updatePlayerStatsHost() {
    const tournamentCode = propsData?.tournamentCode;
    const hostTeamName = propsData?.hostTeam;
  
    const hostTeamPlayersNames = propsData?.hostTeamPlayers.map(player => player.name);
    const battingTeamPlayersNames = battingTeamPlayers.map(player => player.name);
    const isBatting = JSON.stringify(hostTeamPlayersNames) === JSON.stringify(battingTeamPlayersNames);
  
    if (isBatting) {
      console.log(hostTeamName, " IS BATTINGGGGG", battingTeamPlayers);
      const tournamentPath = `/Scorify/tournament/${tournamentCode}`;
      const scoreboardPath = `${tournamentPath}/playerStats/${hostTeamName}`; // New scoreboard path
  
      // Create a reference to the new scoreboard path in Firebase
      const scoreboardRef = database().ref(scoreboardPath);
  
      // Fetch the current scoreboard data from Firebase
      scoreboardRef.once('value', (snapshot) => {
        const currentScoreboard = snapshot.val() || {};
        ...
        battingTeamPlayers.forEach(player => {
          if (
            player.name &&
            player.bat &&
            player.bat.balls !== undefined &&
            player.bat.score !== undefined
          ) {
            // If player has the required properties, update their stats
            currentScoreboard[player.name] = currentScoreboard[player.name] || {}; // Ensure player entry exists
            currentScoreboard[player.name].bat = currentScoreboard[player.name].bat || {}; // Ensure 'bat' property exists
            currentScoreboard[player.name].bat.balls = (currentScoreboard[player.name].bat.balls || 0) + player.bat.balls;
            currentScoreboard[player.name].bat.score = (currentScoreboard[player.name].bat.score || 0) + player.bat.score;
          }
        });
  
        // Update the scoreboard in Firebase
        scoreboardRef.set(currentScoreboard, (error) => {
          if (error) {
            console.error("Error updating player stats:", error);
          } else {
            console.log("Player stats updated in the scoreboard successfully!");
          }
        });
      });
    } else {
      console.log(hostTeamName, " IS BOWLINGGGGG", bowlingTeamPlayers);
      ...
      scoreboardRef.once('value', (snapshot) => {
        ...
        bowlingTeamPlayers.forEach(player => {
            if (
              player.name &&
              player.bowl &&
              player.bowl.balls !== undefined &&
              player.bowl.score !== undefined && // Corrected property name from 'bat.score' to 'bowl.score'
              player.bowl.wickets !== undefined
            ) {
              // If player has the required properties, update their stats
              currentScoreboard[player.name] = currentScoreboard[player.name] || {}; // Ensure player entry exists
              currentScoreboard[player.name].bowl = currentScoreboard[player.name].bowl || {}; // Ensure 'bowl' property exists
              currentScoreboard[player.name].bowl.balls = (currentScoreboard[player.name].bowl.balls || 0) + player.bowl.balls;
              currentScoreboard[player.name].bowl.score = (currentScoreboard[player.name].bowl.score || 0) + player.bowl.score;
              currentScoreboard[player.name].bowl.wickets = (currentScoreboard[player.name].bowl.wickets || 0) + player.bowl.wickets;
              // Corrected property name from 'bat.score' to 'bowl.score'
            }
          });
      ...
      // Handle bowling team stats update similarly
    }
  }
  
  function updatePlayerStatsVisitor() {
    const tournamentCode = propsData?.tournamentCode;
    const hostTeamName = propsData?.visitorTeam;
  
    const hostTeamPlayersNames = propsData?.visitorTeamPlayers.map(player => player.name);
    const battingTeamPlayersNames = battingTeamPlayers.map(player => player.name);
    const isBatting = JSON.stringify(hostTeamPlayersNames) === JSON.stringify(battingTeamPlayersNames);
  ...
  useEffect(() => {
    async function UpdatingTournament(){
    const teamA=propsData?.hostTeam;
    const teamB=propsData?.visitorTeam;
    const tournamentCode=propsData?.tournamentCode
    ...
    if (propsData?.matchType === "Final") {
        const updates = {};
      
        // Define the updates
        updates[`/Scorify/tournament/${tournamentCode}/finalWinner`] = winningTeam;
        updates[`/Scorify/tournament/${tournamentCode}/winningStatement`] = winningStatement;
        updates[`/Scorify/tournament/${tournamentCode}/tournamentFinished`] = matchFinished;
...
if(propsData?.matchType==="Group"){
  ...
  matchPath = `/Scorify/tournament/${tournamentCode}/semiMatches/${matchSnapshot.key}`;
    ...
    const winsRef = database().ref(`/Scorify/tournament/${tournamentCode}/wins`);
          ...
          await winsRef.set(updatedWins);
            ...
            const winsRef = database().ref(`/Scorify/tournament/${tournamentCode}/semiWins`);
                  // Read the current wins value
                  const winsSnapshot = await winsRef.once('value');
                  const currentWins = winsSnapshot.val() || 0; // Default to 0 if there's no value
                  // Increment the wins value by one
                  ...
                  await winsRef.set(updatedWins);
                    ...
                    const winsRef = database().ref(`/Scorify/tournament/${tournamentCode}/finalWin`);
                          // Read the current wins value
                          const winsSnapshot = await winsRef.once('value');
                          const currentWins = winsSnapshot.val() || 0; // Default to 0 if there's no value
                          // Increment the wins value by one
        ...
        if (matchFinished){
            ...
            updatePlayerStatsHost();
            updatePlayerStatsVisitor();
        }
  }, [matchFinished, winningTeam, winningStatement])
  ...
  const handleOkayButtonPress = () => {
    ...
    if (inning === 2) {
      if (score + additionalScore >= target) {
        setWinningTeam(battingTeam);
        setMatchFinished(true);
        setWinningStatement(`${battingTeam} won by ${battingTeamPlayers.length - wickets-1} wickets`)
      }
    ...
    return (
      <View
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.5}
          onPress={() => navigation.navigate("GenerateMatches", {tournamentCode: propsData?.tournamentCode})}>
  ...
  return (
    <View style={{flex: 1, padding: 10, backgroundColor: '#1e1e1e'}}>
      <View style={{flex: 2, padding: 10}}>
        ...
        <Text style={styles.headingText}>{propsData?.tournamentName} 
        
        {propsData?.matchType==="Group" && 
        <Text style={{fontSize: 15}}> ({propsData?.matchType} Match)</Text>
        }

        {propsData?.matchType==="Semi" && 
        <Text style={{fontSize: 15}}> (Semi Final Match)</Text>
        }

        {propsData?.matchType==="Final" && 
        <Text style={{fontSize: 15}}> (Final Match)</Text>
        }

        </Text>
        </View>

        <View style={{...styles.heading, backgroundColor: 'yellow'}}>

        <Text style={{...styles.headingText, color: '#ca3232'}}>Match Code: {propsData?.matchCode}</Text>
        </View>
        <View 
        style={{    
          margin: 5,
